...
const Users = () => {
  ...
  const fetchPendingApprovals = async () => {
    try {
      const response = await axios.get('http://192.168.1.42:3000/api/admin/pending-approvals');
  ...
  const fetchApprovalStats = async () => {
    try {
      const response = await axios.get('http://192.168.1.42:3000/api/admin/approval-stats');
      if (response.data.success) {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await axios.post(`http://192.168.1.42:3000/api/admin/approve-user/${userId}`, {
  ...
  const rejectUser = async () => {
    ...
    try {
      const response = await axios.post(`http://192.168.1.42:3000/api/admin/reject-user/${selectedUserId}`, {
        adminId: 1, // Using integer ID instead of string
